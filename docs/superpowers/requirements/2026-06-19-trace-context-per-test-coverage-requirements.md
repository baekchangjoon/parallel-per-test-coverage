...
| REQ-015 | 서비스 간 병합 리포트 | `DistributedCoverageMergerTest#{perServicePerTestIdReport, multipleTraceIdsOneTestIdWithinServiceMerged, unmappedTraceIdFallsBackToRawPerService}`(서비스 축 병합 메커니즘) + `TraceMergeMainTest`(CLI) + **`LegacyTramDistributedE2E#crossServiceCoverageMerged`**(live Brave 3-서비스: order-web+reservation+ledger downstream 귀속, classCount 6/6/2, 2026-06-20 실측). tainted-spring OTel 경로는 추가 벡터로 가용하나 legacy-tram Brave E2E가 REQ-015 수용기준을 충족. | E2E black-box (live legacy-tram) | 🟢 green |
